...
# from conversation_retrieval_toolkit import ConversationalRetrievalChain, ConversationBufferMemory
...
API_KEY = "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"
...
def get_vector(text_chunks, index_name="index"):
    ...
    return vector_store
def get_conversation_chain(vectorstore,model_kwargs={}):
  """Initialize conversation chain with selectable model."""
  ...
  conversation_chain = ConversationalRetrievalChain.from_llm(hub_llm, memory=memory, retriever=vectorstore.as_retriever())
    ...
    for message in st.session_state.chat_history:
        if is_user_message:
            ...
            if "Helpful Answer:" in message.content:
                answer = message.content.split("Helpful Answer:")[1].strip()
                st.markdown(f'<div style="background-color: #d3d3d3; border-radius: 10px; padding: 10px; margin: 5px 0;"><b>Chatbot:</b> {answer}</div>', unsafe_allow_html=True)
            ...
            with st.spinner("Processing..."):
                ...
                for txt in txt_docs:
                    try:
                        ...
                        text_content = txt.getvalue().decode("latin-1")
